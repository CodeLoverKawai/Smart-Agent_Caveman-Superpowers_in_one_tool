...
test('Hooks Integration', async (t) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'smart-hooks-test-'));
  ...
  const env = {
    ...
    SMART_DEFAULT_MODE: 'ultra',
  };

  const flagPath = path.join(tempDir, '.smart-agent-active');
  const savingsPath = path.join(tempDir, '.smart-stats-savings');

  await t.test('smart-activate.js writes default mode and prints SKILL.md', () => {
    ...
    assert.match(res.stdout, /# smart-agent/);
  });

  await t.test('smart-tracker.js handles /smart-agent off', () => {
    // Write flag
    fs.writeFileSync(flagPath, 'ultra');

    const inputData = JSON.stringify({ prompt: '/smart-agent off' });
  ...
  await t.test('smart-tracker.js handles /smart-agent lite', () => {
    // Write flag
    try { fs.unlinkSync(flagPath); } catch (e) {}

    const inputData = JSON.stringify({ prompt: '/smart-agent lite' });
    const res = spawnSync(process.execPath, [trackerPath], {
      env,
      input: inputData,
      encoding: 'utf8'
  ...
  await t.test('smart-tracker.js handles natural language activation & deactivation', () => {
    // Test activation
    try { fs.unlinkSync(flagPath); } catch (e) {}
    let inputData = JSON.stringify({ prompt: 'please use smart-agent' });
    ...
    // Test deactivation
    inputData = JSON.stringify({ prompt: 'stop smart-agent please' });
  ...
  await t.test('smart-tracker.js appends reinforcement when active', () => {
    // Write flag
    ...
    assert.match(parsed.hookSpecificOutput.additionalContext, /SMART-AGENT ACTIVE \(mode: full\)/);
  });

  await t.test('smart-tracker.js handles /smart off alias', () => {
    // Write flag
    fs.writeFileSync(flagPath, 'ultra');

    const inputData = JSON.stringify({ prompt: '/smart off' });
    const res = spawnSync(process.execPath, [trackerPath], {
      env,
      input: inputData,
      encoding: 'utf8'
  ...
  await t.test('smart-tracker.js handles /smart lite alias', () => {
    // Write flag
    try { fs.unlinkSync(flagPath); } catch (e) {}

    const inputData = JSON.stringify({ prompt: '/smart lite' });
    const res = spawnSync(process.execPath, [trackerPath], {
      env,
      input: inputData,
      encoding: 'utf8'
  ...
  await t.test('smart-tracker.js suspends compression on implementation_plan.md', () => {
    fs.writeFileSync(flagPath, 'full');
  ...
  await t.test('smart-tracker.js handles /smart stats and parses jsonl', () => {
    // Clear any previous savings file or last-parsed file
    try { fs.unlinkSync(savingsPath); } catch (e) {}
    const lastParsedPath = path.join(tempDir, '.smart-stats-last');
    ...
    const inputData = JSON.stringify({ prompt: '/smart stats' });
    const res = spawnSync(process.execPath, [trackerPath], {
      env,
      input: inputData,
      encoding: 'utf8'
  ...
  await t.test('smart-activate.js performs background stats update on SessionStart', () => {
  ...
  await t.test('smart-statusline.sh outputs colored badge and savings', () => {
    // 1. Inactive case
    try { fs.unlinkSync(flagPath); } catch (e) {}
    try { fs.unlinkSync(savingsPath); } catch (e) {}
    ...
    assert.strictEqual(res.status, 0);
    assert.match(res.stdout, /\[SMART:LITE\]/);
